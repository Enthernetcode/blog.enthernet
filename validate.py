...
REQUIRED = {"title", "phase", "status", "summary", "architecture", "how", "commands", "verify", "gotcha", "security", "lesson", "evidence"}
...
for required_link in ("https://core-shield.enthernetservice.com", "https://pinchai.enthernetservice.com", "https://fcs.enthernet.com"):
    if required_link not in all_html:
        raise SystemExit(f"Required live project link missing: {required_link}")

# SEO/entity tripwires: these must survive every future generator refactor.
home_html = (DIST / "index.html").read_text(encoding="utf-8")
seo_required = (
    "Enthernet | Cloud, Cybersecurity &amp; Engineering by Renuel Roberts",
    'rel="canonical" href="https://blog.enthernet.com/"',
    'property="og:site_name" content="Enthernet"',
    'id="enthernet-entity-graph"',
    '"@type":"WebSite"',
    '"@type":"Organization"',
    '"@type":"Person"',
    '"name":"Renuel Roberts"',
    'https://www.linkedin.com/in/renuel-roberts-st-enthernet-code-6571a7241',
    'https://github.com/Enthernetcode',
)
for marker in seo_required:
    if marker not in home_html:
        raise SystemExit(f"Homepage SEO/entity marker missing: {marker}")
for html_file in html_files:
    page = html_file.read_text(encoding="utf-8")
    if page.count('rel="canonical"') != 1:
        raise SystemExit(f"Expected exactly one canonical on {html_file.relative_to(DIST)}")
    if 'name="robots" content="index,follow,max-image-preview:large,max-snippet:-1,max-video-preview:-1"' not in page:
        raise SystemExit(f"Robots indexing directive missing on {html_file.relative_to(DIST)}")
    if 'id="enthernet-entity-graph"' not in page:
        raise SystemExit(f"Entity graph missing on {html_file.relative_to(DIST)}")
...
required_files = [DIST / "index.html", DIST / "100-days" / "index.html", DIST / "projects" / "index.html", DIST / "articles" / "index.html", DIST / "research" / "index.html", DIST / "about" / "index.html", DIST / "contact" / "index.html", DIST / "sitemap.xml", DIST / "rss.xml", DIST / "robots.txt", DIST / "CNAME", DIST / ".nojekyll"]
