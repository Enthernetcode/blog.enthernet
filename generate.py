...
from content import DAY_CONTENT, PUBLISHED_DAYS, LAST_DAY, UNVERIFIED_AWS
...
def main():
    if DIST.resolve().parent != ROOT.resolve():
        raise SystemExit(f"Refusing to wipe DIST outside project root: {DIST}")
    if DIST.exists():
        shutil.rmtree(DIST)
    ...
    roadmap_range = f"Days {min(FUTURE)}–{max(FUTURE)}" if FUTURE else "Roadmap ahead"

    # Homepage
    latest = "".join(day_card(day) for day in range(LAST_DAY, max(0, LAST_DAY - 10), -1))
    project_cards = "".join(
    ...
    home = f'''<section class="hero"><div class="shell hero-grid"><div><span class="eyebrow">Day {LAST_DAY} / 100 · Engineering in public</span><h1>Cloud. Security. Automation. Evidence.</h1><p class="hero-copy">A technical notebook, project archive and public engineering record. The short social post tells the story; this site keeps the implementation, failure modes, verification and field knowledge.</p><div class="actions"><a class="btn btn-primary" href="/100-days/">Explore the journey →</a><a class="btn btn-secondary" href="{SITE['github']}">View GitHub ↗</a></div></div><div class="stats"><div class="stat"><strong>{LAST_DAY} / 100</strong><span>Journey progress</span></div><div class="stat"><strong>CI/CD</strong><span>Current published phase</span></div><div class="stat"><strong>Evidence first</strong><span>Artifacts outrank reconstruction</span></div></div></div></section>
...
<section class="section"><div class="shell"><div class="grid-2"><div><span class="eyebrow">Research notebook</span><h2 style="font-size:clamp(2.3rem,5vw,4.4rem);line-height:1.02">Full Cell Sufficiency</h2><p class="hero-copy">{escape(RESEARCH['summary'])}</p><div class="research-levels">{''.join(f'<span class="tag">{escape(x)}</span>' for x in RESEARCH['levels'])}</div><div class="actions"><a class="btn btn-primary" href="/research/full-cell-sufficiency/">Research record</a><a class="btn btn-secondary" href="{RESEARCH['live']}">Live FCS site ↗</a></div></div><div class="panel"><h3>Evidence stays labeled</h3><p>Established biology, supported inference, hypothesis and speculation are deliberately kept separate so an interesting mechanism does not quietly become an established claim.</p><ul>{''.join(f'<li>{escape(x)}</li>' for x in RESEARCH['areas'])}</ul></div></div></div></section>'''
    ...
    for day in PUBLISHED_DAYS:
        ...
        next_link = f'<a href="{day_href(day+1)}">Day {day+1} →<br><strong>{escape(DAY_CONTENT[day+1]["title"])}</strong></a>' if day < LAST_DAY else f'<a href="/100-days/">Roadmap →<br><strong>{escape(roadmap_range)}</strong></a>'
    ...
    urls += [day_href(d) for d in PUBLISHED_DAYS]
    ...
    items = []
    for day in range(LAST_DAY, max(0, LAST_DAY - 20), -1):
        data = DAY_CONTENT[day]
